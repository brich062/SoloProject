...
public class HomeController {
	...
	@GetMapping("/shop")
	public String shopPage(HttpSession session, Model viewModel) {
		Long userId = (Long) session.getAttribute("user_id");
		viewModel.addAttribute("userLoggedIn", userId);
		viewModel.addAttribute("allProd", this.pServ.findAll());
		return "productsPage.jsp";
	}

	
	 // get mapping for the products page
	 @GetMapping("/products") public String products(HttpSession session, Model viewModel) { 
		 Long userId = (Long) session.getAttribute("user_id");
	 	 viewModel.addAttribute("userLoggedIn", userId); 
	 ...
	 @GetMapping("/products/{id}") public String singleProduct(@PathVariable("id") Long id, Model viewModel, HttpSession session) { 
		 Long userId = (Long)session.getAttribute("user_id"); 
		 viewModel.addAttribute("userLoggedIn", userId); 
		 viewModel.addAttribute("singleProd", this.pServ.getById(id));
	...
	// get mapping for a like
		...
		public String likeIdea(@PathVariable("id") Long id, HttpSession session,Product product) {
			...
			System.out.println(this.pServ.getById(id) + "product");
			//for the redirect
			Long prodId = product.getId();
			Long userId = (Long)session.getAttribute("user_id");
			User liker = this.uServ.findUserById(userId);
			System.out.println(this.uServ.findUserById(userId) + "user");
	...
	public String login(@RequestParam("email") String email, @RequestParam("password") String password,
			RedirectAttributes redAttr, HttpSession session) {
	...
	@GetMapping("/edit/{id}")
	public String getEditPage(@ModelAttribute("user") User user, @PathVariable("id") Long id, Model viewModel,
			HttpSession session) {
		Long userId = (Long) session.getAttribute("user_id");
		viewModel.addAttribute("userLoggedIn", userId);
		viewModel.addAttribute("user__id", session.getAttribute("user_id"));
	...
	@PostMapping("/edit/{id}")
	public String editEntry(@Valid @ModelAttribute("user") User user, BindingResult result, @PathVariable("id") Long id,
	...
	//this is throwing an error from MySQL due to connection between the foreign keys, unsure how to fix, may require extra model and data table
	/*
	 * @GetMapping("/cart/delete/{id}") public String
	 * deleteCartItem(@PathVariable("id") Long id, HttpSession session, CartItem
	 * cartItem) { Long userId = (Long)session.getAttribute("user_id"); Long
	 * itemDelete = cartItem.getId(); this.cServ.deleteFromCart(itemDelete); return
	 * "redirect:/cart"; }
	 */
